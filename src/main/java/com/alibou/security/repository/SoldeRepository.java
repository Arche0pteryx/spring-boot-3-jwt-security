...
public interface SoldeRepository extends JpaRepository<Solde, Long> {
    ...
    @Query(value = "SELECT * FROM soldes t where  t.idUser=?1 and t.idWallet=?2 and t.dateff=(SELECT max(dateff) FROM soldes where  idUser=?1 and idWallet=?2 and idasset=t.idasset)",nativeQuery = true)
