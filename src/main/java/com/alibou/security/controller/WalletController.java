...
public class WalletController {
    ...
    @GetMapping("/wallet")
    public List<Wallet> getAllTransactions(@AuthenticationPrincipal User user) {
        return walletService.getAllWallets(user.getId());
    }

    @GetMapping("/wallet/{id}")
    public Wallet getTransactionById(@PathVariable Long id,@AuthenticationPrincipal User user) {
        return walletService.getWalletById(id,user.getId());
    }

    @PostMapping("/wallet")
    public Wallet createTransaction(@RequestBody Wallet wallet,@AuthenticationPrincipal User user) {
        wallet.setIdUser(user.getId());
        return walletService.createWallet(wallet);
    }

    @PutMapping("/wallet/{id}")
    public Wallet updateTransaction(@PathVariable Long id, @RequestBody Wallet wallet,@AuthenticationPrincipal User user) {
